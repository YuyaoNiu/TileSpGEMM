...
__inline__ __device__
int binary_search_exact_uchar_kernel(const unsigned char * __restrict__ d_array, int l, int r, unsigned char key) 
{ 
    while (l <= r) { 
        int m = l + (r - l) / 2; 
            ...
            int *d_blkid_smem_tny, int *d_blkid_smem_sml, int *d_blkid_smem_lrg, int *d_blkid_smem_dns)
{
    ...
    gettimeofday(&t2, NULL);
    double time_kernel = (t2.tv_sec - t1.tv_sec) * 1000.0 + (t2.tv_usec - t1.tv_usec) / 1000.0;
    printf("CUDA step3 kernel = %.2f ms, numblkC = %i\n", time_kernel, numblkC);
    ...
    *nnzC = nbc;
    cudaDeviceSynchronize();
    gettimeofday(&t2, NULL);
    time_kernel = (t2.tv_sec - t1.tv_sec) * 1000.0 + (t2.tv_usec - t1.tv_usec) / 1000.0;
    printf("CUDA scan kernel = %.2f ms, nnzC = %i\n", time_kernel, nbc);
...
__global__
void stir_spgemm_step4_cuda_kernel_smem_v3_scalar
                             (int *d_blkrowptrA,
                            const int* __restrict__ d_blkcolidxA,
                            int *d_nnzb_A,
                            MAT_VAL_TYPE *d_blkcsr_Val_A,
                            ...
                            int *d_blkid)
{
    ...
    if (global_id >= numblkC) return;
    global_id = d_blkid[global_id];

    const int nnzcstart = d_nnzb_C[global_id];
    const int blknnzctotal = d_nnzb_C[global_id+1] - nnzcstart;
    if (!blknnzctotal) return;

    //const int local_warp_id = threadIdx.x >> 5;//threadIdx.x / WARP_SIZE;
    const int local_id = threadIdx.x;
    __shared__ unsigned char s_blkcsr_Idx_C[WARP_SIZE * WARP_PER_BLOCK * SMEM_SIZE1];
    __shared__ MAT_VAL_TYPE s_blkcsr_Val_C[WARP_SIZE * WARP_PER_BLOCK * SMEM_SIZE1];
    //__shared__ unsigned char s_blkcsr_Ptr_C[WARP_SIZE * WARP_PER_BLOCK * BLOCK_SIZE];
    unsigned char *s_blkcsr_Idx_C_local = &s_blkcsr_Idx_C[local_id * SMEM_SIZE1];
    MAT_VAL_TYPE *s_blkcsr_Val_C_local = &s_blkcsr_Val_C[local_id * SMEM_SIZE1];
    //unsigned char *s_blkcsr_Ptr_C_local = &s_blkcsr_Ptr_C[local_id * BLOCK_SIZE];
    //unsigned char s_blkcsr_Idx_C_local[SMEM_SIZE1];
    //MAT_VAL_TYPE s_blkcsr_Val_C_local[SMEM_SIZE1];
    unsigned char s_blkcsr_Ptr_C_local[BLOCK_SIZE];

    //__shared__ unsigned char s_csrRowPtrB[WARP_SIZE * WARP_PER_BLOCK * BLOCK_SIZE];
    //unsigned char *s_csrRowPtrB_local = &s_csrRowPtrB[local_id * BLOCK_SIZE];
    unsigned char s_csrRowPtrB_local[BLOCK_SIZE];
    
    //__shared__ int s_matched_posa[WARP_SIZE * WARP_PER_BLOCK * SMEM_SIZE2];
    //__shared__ int s_matched_posb[WARP_SIZE * WARP_PER_BLOCK * SMEM_SIZE2];
    //__shared__ int s_matchedcnt[WARP_SIZE * WARP_PER_BLOCK];
    
    //int *s_matched_posa_local = &s_matched_posa[local_id * SMEM_SIZE2];
    //int *s_matched_posb_local = &s_matched_posb[local_id * SMEM_SIZE2];
    //int *s_matchedcnt_local = &s_matchedcnt[local_id];

    //const int lane_id = (WARP_SIZE - 1) & threadIdx.x;

    #pragma unroll
    for (int i = 0; i < SMEM_SIZE1; i++)
    ...
    #pragma unroll
    for (int i = 0; i < BLOCK_SIZE; i++)
        s_blkcsr_Ptr_C_local[i] = d_blkcsr_Ptr_C[global_id * BLOCK_SIZE + i];
    //__syncthreads();

    //unsigned int maskc = lane_id < BLOCK_SIZE ? d_blkmaskC[global_warp_id * BLOCK_SIZE + lane_id] : 0;//s_maskc_local[lane_id];
    unsigned int maskc[BLOCK_SIZE];
    #pragma unroll
    for (int i = 0; i < BLOCK_SIZE; i++)
        maskc[i] = d_blkmaskC[global_id * BLOCK_SIZE + i];

    //unsigned char blknnzcstart = lane_id < BLOCK_SIZE ? s_blkcsr_Ptr_C_local[lane_id] : 0;
    unsigned char blknnzcstart[BLOCK_SIZE];
    #pragma unroll
    for (int i = 0; i < BLOCK_SIZE; i++)
        blknnzcstart[i] = s_blkcsr_Ptr_C_local[i];
    ...
    for (int li = 0; li < BLOCK_SIZE; li++)
    {
        ...
        for (int i = 0; i < BLOCK_SIZE; i++)
        {
            int idx = ((maskc[li] >> BLOCK_SIZE - i - 1) & 0x1) == 1 ? i : -1;
            if (idx != -1)
            {
                s_blkcsr_Idx_C_local[blknnzcstart[li] + cnt] = idx;
                cnt++;
            }
        }
    }

    const int blki = d_blkrowidxC[global_id];
    const int blkj = d_blkcolidxC[global_id];
    ...
    */
    {
        ...
        while(posa < lena && posb < lenb)
        {
            ...
            if (idxa == idxb)
            {
                ...
                #pragma unroll
                for (int li = 0; li < BLOCK_SIZE; li++)
                    s_csrRowPtrB_local[li] = ld_gbl_uchar(d_blkcsr_Ptr_B+(bbase+posb)*BLOCK_SIZE+li);
...
template <int SMEM_SIZE1, int SMEM_SIZE2, int THREADS_PER_ROW>
__global__
void stir_spgemm_step4_cuda_kernel_smem_v3_halfwarp
                             (int *d_blkrowptrA,
                            const int* __restrict__ d_blkcolidxA,
                            int *d_nnzb_A,
                            MAT_VAL_TYPE *d_blkcsr_Val_A,
                            ...
                            int *d_blkid)
{
    ...
    __shared__ unsigned char s_blkcsr_Idx_C[HALFWARP_PER_BLOCK * SMEM_SIZE1];
    __shared__ MAT_VAL_TYPE s_blkcsr_Val_C[HALFWARP_PER_BLOCK * SMEM_SIZE1];
    ...
    __shared__ int s_matched_posa[HALFWARP_PER_BLOCK * SMEM_SIZE2];
    __shared__ int s_matched_posb[HALFWARP_PER_BLOCK * SMEM_SIZE2];
    ...
    if (lena < lenb)
    {
        for (int i = lane_id; i < lena; i+= HALFWARP_SIZE)
        {
            int idxa = d_blkcolidxA[abase+i];
            int res = binary_search_exact_kernel(d_blkrowidxB+bbase, 0, lenb-1, idxa);
            if (res != -1)
    ...
    {
        for (int i = lane_id; i < lenb; i+= HALFWARP_SIZE)
        {
            int idxb = ld_gbl_int32(d_blkrowidxB+bbase+i);
            int res = binary_search_exact_kernel(d_blkcolidxA+abase, 0, lena-1, idxb);
            if (res != -1)
...
template <int SMEM_SIZE, int THREADS_PER_ROW>
__global__
void stir_spgemm_step4_cuda_kernel_dns_noatomic
                             (int *d_blkrowptrA,
                            const int* __restrict__ d_blkcolidxA,
                            int *d_nnzb_A,
                            MAT_VAL_TYPE *d_blkcsr_Val_A,
...
template <int SMEM_SIZE, int THREADS_PER_ROW>
__global__
void stir_spgemm_step4_cuda_kernel_dns_noatomic_halfwarp
                             (int *d_blkrowptrA,
                            const int* __restrict__ d_blkcolidxA,
                            int *d_nnzb_A,
                            MAT_VAL_TYPE *d_blkcsr_Val_A,
                            ...
                            int *d_blkid)
{
    ...
    __shared__ int s_matched_posa[HALFWARP_PER_BLOCK * SMEM_SIZE];
    __shared__ int s_matched_posb[HALFWARP_PER_BLOCK * SMEM_SIZE];
    __shared__ int s_matchedcnt[HALFWARP_PER_BLOCK];
    ...
    s_blkcsr_Val_C_local[lane_id] = 0.0;
    s_blkcsr_Val_C_local[16 + lane_id] = 0.0;
    s_blkcsr_Val_C_local[32 + lane_id] = 0.0;
    s_blkcsr_Val_C_local[48 + lane_id] = 0.0;
    s_blkcsr_Val_C_local[64 + lane_id] = 0.0;
    s_blkcsr_Val_C_local[80 + lane_id] = 0.0;
    s_blkcsr_Val_C_local[96 + lane_id] = 0.0;
    s_blkcsr_Val_C_local[112 + lane_id] = 0.0;
...
template <int SMEM_SIZE, int THREADS_PER_ROW>
__global__
void stir_spgemm_step4_cuda_kernel_dns_halfwarp
                             (int *d_blkrowptrA,
                            const int* __restrict__ d_blkcolidxA,
                            int *d_nnzb_A,
                            MAT_VAL_TYPE *d_blkcsr_Val_A,
...
// tiny : 1 - 32
    ...
    stir_spgemm_step4_cuda_kernel_smem_v3<SMEM_TNY_TH,32,4><<< num_blocks, num_threads>>>
                                (d_blkrowptrA, d_blkcolidxA, d_nnzb_A, d_blkcsr_Val_A, d_blkcsr_Col_A, d_blkcsr_Ptr_A,
                                 blkmA, blknA, numblkA, nnzA, 
                                 d_blkcolptrB, d_blkrowidxB, d_nnzb_B, d_blkcsr_Val_B, d_blkcsr_Col_B, d_blkcsr_Ptr_B,
                                 blkmB, blknB, numblkB, nnzB, 
...
// large : 65 - 128
    ...
    stir_spgemm_step4_cuda_kernel_smem_v3<SMEM_LRG_TH,32,4><<< num_blocks, num_threads >>>
                             (d_blkrowptrA, d_blkcolidxA, d_nnzb_A, d_blkcsr_Val_A, d_blkcsr_Col_A, d_blkcsr_Ptr_A,
                              blkmA, blknA, numblkA, nnzA, 
                              d_blkcolptrB, d_blkrowidxB, d_nnzb_B, d_blkcsr_Val_B, d_blkcsr_Col_B, d_blkcsr_Ptr_B,
                              blkmB, blknB, numblkB, nnzB, 
...
// dns : 129 - dns
    ...
    gettimeofday(&tt1, NULL);
     ...
     stir_spgemm_step4_cuda_kernel_dns_noatomic_halfwarp<128,8><<< num_blocks, num_threads >>>
                             (d_blkrowptrA, d_blkcolidxA, d_nnzb_A, d_blkcsr_Val_A, d_blkcsr_Col_A, d_blkcsr_Ptr_A,
                              blkmA, blknA, numblkA, nnzA, 
                              d_blkcolptrB, d_blkrowidxB, d_nnzb_B, d_blkcsr_Val_B, d_blkcsr_Col_B, d_blkcsr_Ptr_B,
                              blkmB, blknB, numblkB, nnzB, 
    ...
    if (blksmem_tny_cnt)
    {
        ...
        /*stir_spgemm_step4_cuda_kernel_smem_v2<SMEM_TNY_TH,32><<< num_blocks, num_threads, 0, stream[0] >>>
                                 (d_blkrowptrA, d_blkcolidxA, d_nnzb_A, d_blkcsr_Val_A, d_blkcsr_Col_A, d_blkcsr_Ptr_A,
                                  ...
                                  d_blkcsr_Col_C,d_blkcsr_Val_C,
                                  d_nnzb_C, d_blkmaskC, blksmem_tny_cnt, d_blkid_smem_tny);*/
        stir_spgemm_step4_cuda_kernel_smem_v3<SMEM_TNY_TH,32,4><<< num_blocks, num_threads, 0, stream[1] >>>
                                  (d_blkrowptrA, d_blkcolidxA, d_nnzb_A, d_blkcsr_Val_A, d_blkcsr_Col_A, d_blkcsr_Ptr_A,
                                   blkmA, blknA, numblkA, nnzA, 
                                   d_blkcolptrB, d_blkrowidxB, d_nnzb_B, d_blkcsr_Val_B, d_blkcsr_Col_B, d_blkcsr_Ptr_B,
                                   blkmB, blknB, numblkB, nnzB, 
...
// large : 65 - 128
   ...
   {
     num_blocks = ceil((double)blksmem_lrg_cnt/(double)WARP_PER_BLOCK);
    stir_spgemm_step4_cuda_kernel_smem_v3<SMEM_LRG_TH,32,4><<< num_blocks, num_threads, 0, stream[2] >>>
                             (d_blkrowptrA, d_blkcolidxA, d_nnzb_A, d_blkcsr_Val_A, d_blkcsr_Col_A, d_blkcsr_Ptr_A,
                              blkmA, blknA, numblkA, nnzA, 
                              d_blkcolptrB, d_blkrowidxB, d_nnzb_B, d_blkcsr_Val_B, d_blkcsr_Col_B, d_blkcsr_Ptr_B,
                              blkmB, blknB, numblkB, nnzB, 
...
// dns : 129 - dns
   ...
   if (blksmem_dns_cnt)
   {
     num_threads = WARP_SIZE * WARP_PER_BLOCK;
     num_blocks = ceil((double)blksmem_dns_cnt/(double)WARP_PER_BLOCK);
     stir_spgemm_step4_cuda_kernel_dns_noatomic_halfwarp<128,8><<< num_blocks, num_threads , 0, stream[3] >>>
                             (d_blkrowptrA, d_blkcolidxA, d_nnzb_A, d_blkcsr_Val_A, d_blkcsr_Col_A, d_blkcsr_Ptr_A,
                              blkmA, blknA, numblkA, nnzA, 
                              d_blkcolptrB, d_blkrowidxB, d_nnzb_B, d_blkcsr_Val_B, d_blkcsr_Col_B, d_blkcsr_Ptr_B,
                              blkmB, blknB, numblkB, nnzB, 
